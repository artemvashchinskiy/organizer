...
export function createExportFilename(): string {
    const now = new Date();
    return `FocusOS-${String(now.getDate()).padStart(2,"0")}.${String(now.getMonth()+1).padStart(2,"0")}.${String(now.getFullYear()).slice(2)}-${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}.json`;
}
        ...
        input.onchange=()=>{
            ...
            reader.onload=()=>{
                try{
                    ...
                    if(!Array.isArray(notes)) throw new Error("Invalid JSON");
                    resolve({ notes, filename:file.name });
...
export function mergeImportedNotes(current:Note[],imported:Note[]):Note[]{
    const result=[...current];

    imported.forEach(importedNote=>{
        const existing=current.find(note=>note.id===importedNote.id);
        if(!existing){ result.push(importedNote); return; }
        if(JSON.stringify(existing)===JSON.stringify(importedNote)) return;
        ...
        if(!group){
            group=`D-${Date.now()}`;
            duplicateNumber=Math.max(0,...result.map(note=>note.duplicateNumber ?? 0))+1;
            const colors=["#ffe082","#90caf9","#a5d6a7","#ce93d8","#ffab91","#80cbc4","#b39ddb","#fff59d"];
            duplicateColor=colors[(duplicateNumber-1)%colors.length];
