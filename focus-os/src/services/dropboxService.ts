...
export function isDropboxAuthCallback(): boolean {
    const params = new URLSearchParams(window.location.search);
    const code = params.get("code");
    const state = params.get("state");
    const savedState = sessionStorage.getItem("dropbox-state");
    return Boolean(code && state && savedState && state === savedState);
}
