...
import {  uploadBackup, isDropboxAuthCallback} from "./services/dropboxService";
...
import useOneDriveBackup from
    "./hooks/useOneDriveBackup";
...
function App(){
    ...
    const [freeNotes,setFreeNotes] = useLocalStorage<FreeNote[]>(
        user ? `free-notes-${user.id}` : "free-notes-empty",
        []
    );

    const [matrixTasks,setMatrixTasks] = useLocalStorage<EisenhowerTask[]>(
    ...
    const [licenseOpen,setLicenseOpen] =
    useState(false);
...
useEffect(() => {
    ...
    function addFreeNote(text:string){
        ...
        const now = Date.now();
        setFreeNotes(prev => [
            { id: now, text: value, createdAt: now, updatedAt: now },
            ...prev
        ]);
        touch();
    }
    ...
    function updateFreeNote(id:number,text:string){
        ...
        setFreeNotes(prev =>
            prev.map(note =>
                note.id === id
                    ? { ...note, text:value, updatedAt:Date.now() }
                    : note
            )
        );
        touch();
    ...
    function deleteFreeNote(id:number){
        setFreeNotes(prev => prev.filter(note => note.id !== id));
        touch();
    }
    ...
    function addMatrixTask(text:string,quadrant:EisenhowerQuadrant){
        const value = text.trim();
        if(!value) return;
        setMatrixTasks(prev => [
            ...prev,
            { id:Date.now(), text:value, quadrant, completed:false, createdAt:Date.now() }
        ]);
        touch();
    }
    ...
    function updateMatrixTask(id:number,text:string){
        const value = text.trim();
        if(!value) return;
        setMatrixTasks(prev =>
            prev.map(task =>
    ...
    function moveMatrixTask(id:number,direction:-1|1){
        setMatrixTasks(prev => {
            ...
            const localIndex = sameQuadrant.findIndex(item=>item.task.id === id);
            const targetLocalIndex = localIndex + direction;
            if(targetLocalIndex < 0 || targetLocalIndex >= sameQuadrant.length) return prev;
            const targetIndex = sameQuadrant[targetLocalIndex].idx;
            const next = [...prev];
    ...
    function deleteMatrixTask(id:number){
        setMatrixTasks(prev => prev.filter(task => task.id !== id));
        touch();
    }

    function addExpense(amount:number,category:ExpenseCategory,description:string,date:string){
        setExpenses(prev => [
            ...prev,
            { id:Date.now(), amount, category, description, date, createdAt:Date.now() }
        ].sort((a,b) => b.date.localeCompare(a.date) || b.createdAt-a.createdAt));
        touch();
    }

    function updateExpense(id:number,amount:number,category:ExpenseCategory,description:string,date:string){
        if(!Number.isFinite(amount) || amount <= 0) return;
        setExpenses(prev =>
            prev.map(expense =>
                expense.id === id
                    ? { ...expense, amount, category, description:description.trim(), date }
                    : expense
            )
        );
        touch();
    ...
    function moveExpense(id:number,direction:-1|1){
        setExpenses(prev => {
    ...
    function moveNote(id:number,direction:-1|1){
        setNotes(prev => {
            const index = prev.findIndex(note => note.id === id);
            const target = index + direction;
            if(index < 0 || target < 0 || target >= prev.length) return prev;
    ...
    return(
        ...
        <div className="app">
            ...
            <div className="topbar">
                <button className="back" onClick={handleLogout} aria-label="Logout">◀</button>
                <div className="title">React Calendar Timer</div>
                <button className="back" onClick={()=>setSidebarOpen(true)} aria-label="Open system and cloud sidebar" title="System & Cloud">☰</button>
            </div>
